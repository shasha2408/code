...
class LSTM_Var_Autoencoder(object):
    def __init__(
        ...
        if not intermediate_dim or not z_dim or not n_dim:
            raise ValueError(
                "You should set intermediate_dim, z_dim"
        ...
        weights = {
            "z_mean": tf.get_variable(
                "z_mean",
                shape=[self.intermediate_dim, self.z_dim],
        ...
        biases = {
            "z_mean_b": tf.get_variable(
                "b_mean", shape=[self.z_dim], initializer=tf.zeros_initializer()
            ),
            "z_std_b": tf.get_variable(
                "b_log_sigma", shape=[self.z_dim], initializer=tf.zeros_initializer()
            ),
        }
        ...
        # LSTMStateTuples.
        self.z_mean = tf.add(
            tf.matmul(outputs[:, -1, :], weights["z_mean"]), biases["z_mean_b"]
        )
        self.z_sigma = tf.nn.softplus(
            tf.add(
                tf.matmul(outputs[:, -1, :], weights["log_sigma"]), biases["z_std_b"]
            )
        )
            ...
            if self.stateful:
                with tf.variable_scope("lstm_decoder_stateful"):
                    rnn_layers_ = [
                        ...
                        for size in [self.intermediate_dim, n_dim]
                    ]
    ...
    def _create_loss_optimizer(self, opt, **param):
        with tf.name_scope("MSE"):
            reconstr_loss = tf.reduce_sum(
                tf.losses.mean_squared_error(self.x, self.x_reconstr_mean)
            )
        with tf.name_scope("KL_divergence"):
            latent_loss = -0.5 * tf.reduce_sum(
                1 + self.z_sigma - self.z_mean ** 2 - tf.exp(self.z_sigma), 1
            )
